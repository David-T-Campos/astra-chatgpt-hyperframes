...
// Verify the committed source bundle first, then regenerate the text-cleared plates from the original reference.
run(process.execPath,['scripts/check.mjs']);run('ffmpeg',['-version']);run('ffprobe',['-version']);
run('python3',['scripts/build-reference-plates.py']);
