...
// Motion is deliberately slowed to ~2/3 of the reference speed, with short holds between scenes.
...
// and adds little to the story. The saved time becomes slower motion and breathing room elsewhere.
const motionFps=16;
const pacing=[{a:0,b:21,hold:.75},{a:22,b:44,hold:.5},{a:45,b:67,hold:.75},{a:68,b:71,hold:0},{a:72,b:106,hold:.75},{a:134,b:160,hold:2},{a:161,b:179,hold:.6875}];
function sourceFrameAt(time){let t=clamp(time/15)*15,cursor=0;for(const seg of pacing){const motion=(seg.b-seg.a+1)/motionFps;if(t<cursor+motion)return Math.min(seg.b,seg.a+Math.floor((t-cursor)*motionFps));cursor+=motion;if(t<cursor+seg.hold)return seg.b;cursor+=seg.hold}return 179}
function clearLegacyCloseupCopy(){ctx.save();ctx.fillStyle='#ffffff';ctx.fillRect(1080,205,840,135);ctx.restore()}
async function drawFrame(time){await ready;const f=sourceFrameAt(time),track=window.referenceTracks[f],img=await plate(f);ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,1920,1080);ctx.drawImage(img,0,0);
 if(f<22){
  ...
  clearLegacyCloseupCopy();const center=Math.min(1180,Math.max(740,track.left+660));const headingBlur=f<76?(76-f)*1.6:0;type('Give ARGUS something to engineer',center,292,52,'#181a1b','center',400,headingBlur,920);const x=track.left+85,target='Design a 30 m pedestrian bridge',n=Math.max(0,Math.floor((f-72)*.95)),s=target.slice(0,n),blur=f<76?(76-f)*2:0;type(s,x,513,58,'#181a1b','left',400,blur);ctx.save();ctx.font='400 58px Switzer';const w=ctx.measureText(s).width;ctx.fillStyle='#40a2d4';ctx.filter=blur?`blur(${blur}px)`:'none';ctx.fillRect(x+w+10,466,7,52);ctx.restore();
 }else if(f<161){
