...
const pacing=[{a:0,b:21,hold:1},{a:22,b:44,hold:.75},{a:45,b:67,hold:1},{a:68,b:71,hold:0},{a:72,b:106,hold:1.25},{a:107,b:133,hold:.5},{a:134,b:160,hold:2},{a:161,b:179,hold:1}];
...
function cleanCloseupHeading(){ctx.save();ctx.fillStyle='rgba(255,255,255,.995)';ctx.fillRect(560,170,1360,175);ctx.restore()}
async function drawFrame(time){await ready;const f=sourceFrameAt(time),track=window.referenceTracks[f],img=await plate(f);ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,1920,1080);ctx.drawImage(img,0,0);
 if(f<22){
  ...
  const center=track.left+660;cleanCloseupHeading();const headingBlur=f<76?(76-f)*1.6:0;type('Give ARGUS something to engineer',center,292,52,'#181a1b','center',400,headingBlur,920);const x=track.left+85,target='Design a 30 m pedestrian bridge',n=Math.max(0,Math.floor((f-72)*.95)),s=target.slice(0,n),blur=f<76?(76-f)*2:0;type(s,x,513,58,'#181a1b','left',400,blur);ctx.save();ctx.font='400 58px Switzer';const w=ctx.measureText(s).width;ctx.fillStyle='#40a2d4';ctx.filter=blur?`blur(${blur}px)`:'none';ctx.fillRect(x+w+10,466,7,52);ctx.restore();
 }else if(f<134){
  const p=(f-107)/26,top=378-78*p,baseline=top+108,right=1410-105*Math.min(1,p*1.35),blur=f<111?(111-f)*1.6:f>130?(f-130)*1.5:0;ctx.save();ctx.fillStyle='rgba(255,255,255,.997)';ctx.fillRect(0,top+22,955,118);ctx.restore();type('Design a 30 m pedestrian bridge',right-1460,baseline,58,'#202020','left',400,blur);
 }else if(f<161){
