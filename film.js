...
async function drawFrame(time){await ready;const f=Math.min(359,Math.max(0,Math.round(time*24)))%180,track=window.referenceTracks[f];const img=await plate(f);ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,1920,1080);ctx.drawImage(img,0,0);
 if(f<22){
  ...
  type('ARGUS Engineer',0,91,237,g,'center',600,Math.max(0,7-f*1.5),1310);ctx.restore();
  ...
  type('Your autonomous engineer for the physical world',center,track.headingY,54*scale,color,'center',400,blur);ctx.restore();
  ctx.save();ctx.globalAlpha=clamp((f-46)/7);ctx.font=`${36*scale}px monospace`;ctx.textAlign='left';ctx.textBaseline='alphabetic';ctx.fillStyle='#828282';ctx.filter=blur?`blur(${blur}px)`:'none';ctx.fillText('Design a 30 m pedestrian bridge',left+43*scale,540);ctx.restore();
 }else if(f>=72&&f<107){
  ...
  type('Give ARGUS something to engineer',h?h[0]:488,y+214,45,'#7d7d7d','left',400,blur);
 ...
 }else if(f>=161){type('ARGUS Engineer.',966,553,57,'#0d2d48','center',600)}
}
