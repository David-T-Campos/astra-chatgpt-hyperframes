...
function type(s,x,y,size,color='#202020',align='left',weight=400,blur=0,maxWidth){ctx.save();ctx.font=`${weight} ${size}px Switzer,Arial`;ctx.textBaseline='alphabetic';ctx.textAlign=align;ctx.fillStyle=color;ctx.filter=blur?`blur(${blur}px)`:'none';if(maxWidth)ctx.fillText(s,x,y,maxWidth);else ctx.fillText(s,x,y);ctx.restore()}
function gradient(x0,x1,a,b){const g=ctx.createLinearGradient(x0,0,x1,0);g.addColorStop(0,a);g.addColorStop(1,b);return g}
async function drawFrame(time){await ready;const f=Math.min(359,Math.max(0,Math.round(time*24)))%180,track=window.referenceTracks[f];const img=await plate(f);ctx.setTransform(1,0,0,1,0,0);ctx.clearRect(0,0,1920,1080);ctx.drawImage(img,0,0);
 if(f<22){
  ...
  type('Ask ChatGPT',0,91,237,g,'center',600,Math.max(0,7-f*1.5),1310);ctx.restore();
  ...
  type('Your AI assistant for everyday ideas',center,track.headingY,54*scale,color,'center',400,blur);ctx.restore();
  ctx.save();ctx.globalAlpha=clamp((f-46)/7);ctx.font=`${36*scale}px monospace`;ctx.textAlign='left';ctx.textBaseline='alphabetic';ctx.fillStyle='#828282';ctx.filter=blur?`blur(${blur}px)`:'none';ctx.fillText('Write me a poem about possibility',left+43*scale,540);ctx.restore();
 }else if(f>=72&&f<107){
  const x=track.left+85;const n=Math.max(0,Math.floor((f-72)*.67));const s='Compare ideas with ChatGPT'.slice(0,n);
  const blur=f<76?(76-f)*2:0;type(s,x,513,72,'#181a1b','left',400,blur);
  ctx.save();ctx.font='400 72px Switzer';const w=ctx.measureText(s).width;ctx.fillStyle='#40a2d4';ctx.filter=blur?`blur(${blur}px)`:'none';ctx.fillRect(x+w+12,457,8,65);ctx.restore();
 }else if(f>=134&&f<161){
  const y=track.resultOffset,blur=Math.max(0,(138-f)*1.7);const h=track.heading;
  type('Ask ChatGPT what you need to know',h?h[0]:488,y+214,45,'#7d7d7d','left',400,blur);
  const rows=[['Understand complex topics','Get clear explanations, one step at a time.'],['Find the details you need','Quickly explore ideas and useful information.'],['Compare ideas quickly','See different possibilities side by side.']];
  rows.forEach((r,i)=>{type(r[0],807,y+391+i*196,55,'#2b2b2b','left',400,blur);type(r[1],807,y+442+i*196,41,'#858585','left',400,blur)});
 }else if(f>=161){type('Ask ChatGPT.',966,553,57,'#0d2d48','center',600)}
}
